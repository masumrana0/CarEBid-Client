...
import { useGetAllPendingJobQuery } from "@/Redux/api/jobApi";
import { useAppDispatch, useAppSelector } from "@/Redux/hooks";
import { generateJobQuery, setLimit } from "@/Redux/Slices/jobQuerySlice";
import React, { useEffect, useState } from "react";
import JobCardP from "./JobCardP";

const PendingJobPart = () => {
  ...
  const { data, isLoading } = useGetAllPendingJobQuery(query);
    ...
    <div className=" space-y-3  w-full container mx-auto">
      ...
      {isLoading ? (
        ...
        <div className=" grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5   ">
          {allJobs?.map((job: IJob) => <JobCardP job={job} key={job?._id} />)}
        </div>
      )}
    </div>
  );
